...
bool ReadFileToMemory(const std::wstring& filePath, std::vector<BYTE>& outBuffer)
{
    ...
    if (!inputFile.read(reinterpret_cast<PCHAR>(outBuffer.data()), size))
        return false;
...
bool ValidatePEHeaders(PBYTE localImage)
{
    ...
    if (ntHeaders->FileHeader.Machine != CURRENT_ARCH) return false;
    return true;
}
...
PBYTE AllocateAndWriteHeaders(HANDLE process, PBYTE localImage, PIMAGE_NT_HEADERS& ntHeaders)
{
    ...
    PBYTE remoteImage = (PBYTE)VirtualAllocEx(process, nullptr, ntHeaders->OptionalHeader.SizeOfImage, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
    if (remoteImage == nullptr) return nullptr;

    DWORD oldP = 0;
    VirtualProtectEx(process, remoteImage, ntHeaders->OptionalHeader.SizeOfImage, PAGE_EXECUTE_READWRITE, &oldP);

    if (!WriteProcessMemory(process, remoteImage, localImage, (SIZE_T)ntHeaders->OptionalHeader.SizeOfHeaders, nullptr))
    {
...
bool WriteSections(HANDLE process, PBYTE localImage, PBYTE remoteImage, PIMAGE_NT_HEADERS ntHeaders)
{
    ...
    for (int i = 0; i < ntHeaders->FileHeader.NumberOfSections; ++i)
    {
        ...
        if (!WriteProcessMemory(process, (LPVOID)dst, (LPVOID)src, size, nullptr))
            return false;
    }
}

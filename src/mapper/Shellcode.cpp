    ...
    ManualMapData* mapData, RtlAddFunctionTableFunc rtlAddFunctionTable)
{
    ...
    if (!rtlAddFunctionTable(
        ...
        (DWORD64)(imageBase)))
    {
        mapData->errorCode = ManualMapError::EXCEPTION_SUPPORT_FAILED;
        return false;
    }
...
void __stdcall Shellcode(ManualMapData* mapData)
{
    ...
    if (!ProcessRelocations(imageBase, optionalHeader, mapData)) return;

    if (!ProcessImports(imageBase, optionalHeader, mapData, mapData->loadLibrary, mapData->getProcAddress)) return;

    if (mapData->enableTlsCallbacks)
    {
        if (!ProcessTlsCallbacks(imageBase, optionalHeader, mapData)) return;
    }

#ifdef _WIN64
    if (!ProcessExceptions(imageBase, optionalHeader, mapData, mapData->rtlAddFunctionTable))
    {
        mapData->moduleHandle = (HINSTANCE)EXCEPTION_SETUP_FAILED_VALUE;
        return;
    }
#endif
    ...
    if (dllMainResult)
    {
        mapData->errorCode = ManualMapError::SUCCESS;
		mapData->moduleHandle = (HINSTANCE)imageBase;
    }
    else
    {
        mapData->errorCode = ManualMapError::DLL_MAIN_FAILED;
        mapData->moduleHandle = nullptr;
    }
}
...
bool WaitForInjectionResult(HANDLE processHandle, HANDLE threadHandle, HINSTANCE& outModule, LPVOID mapData, DWORD timeoutMs)
{
    ...
    {
        logs::LogError(L"[!] Timeout: Shellcode is hanging!");
        return false;
    }

    if (!ReadProcessMemory(processHandle, mapData, &dataChecked, sizeof(dataChecked), nullptr))
    {
        logs::LogError(L"[-] Failed to read ManualMapData back from process.");
        return false;
    }

    if (dataChecked.moduleHandle != nullptr)
    {
		logs::LogDebug(L"[*] Shellcode reported module handle: {:p}", (void*)dataChecked.moduleHandle);
